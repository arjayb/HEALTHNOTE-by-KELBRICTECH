...
export async function saveConfirmedCapture({ sourceType, captureOrigin, clinicalDate, documentPrintAt, rawOcrText, confirmedFields, sourceRef, sourceBlob, observedAt: suppliedObservedAt }) {
  ...
  const anyUserEdited = confirmedFields.some((field) => field.userEdited);

  const sourceDocument = {
    id: sourceDocumentId, controlNumber, controlNumberFormatted: formatControlNumber(controlNumber), sourceType, captureOrigin,
    sourceRef: sourceRef || null, sourceBlob: sourceBlob || null, sourceMimeType: sourceBlob?.type || null,
    rawOcrText: rawOcrText || "", clinicalDate: clinicalDate || null, documentPrintAt: documentPrintAt || null,
    observedAt, addedAt: now,
  ...
  for (const field of confirmedFields) {
    ...
    const verificationState = isManualEntry || field.userEdited ? "UNVERIFIED" : sourceVerificationState;

    observations.push({
      id: crypto.randomUUID(), sourceDocumentId, canonicalFieldId: field.canonicalFieldId, rawLabel: field.rawLabel,
      normalizedLabel: canonicalField.label, value: field.value, unit: field.unit, referenceRange: field.referenceRange || null,
      verificationState, isBaseline: baseline, clinicalDate: clinicalDate || null, observedAt, addedAt: now,
      sourceType, controlNumber, controlNumberFormatted: formatControlNumber(controlNumber), createdAt: now, updatedAt: now,
      correctionHistory: [], userEditedFromSource: Boolean(field.userEdited),
    });
  }
...
export async function correctObservation(observationId, { newValue, newUnit, reason }) {
  ...
  const auditEntry = { id: crypto.randomUUID(), observationId, previousValue: obs.value, previousUnit: obs.unit, newValue, newUnit, reason: reason || null, correctedAt: new Date().toISOString() };
  const updatedObs = { ...obs, value: newValue, unit: newUnit, verificationState: "UNVERIFIED", userEditedFromSource: true, updatedAt: auditEntry.correctedAt, correctionHistory: [...(obs.correctionHistory || []), auditEntry.id] };
...
export async function deleteSourceDocument(sourceDocumentId) {
  ...
  for (const obs of observations) {
    if (obs.isBaseline) await tx.objectStore("baselineTombstone").put({ canonicalFieldId: obs.canonicalFieldId, originalBaselineObservationId: obs.id, originalBaselineSourceDocumentId: sourceDocumentId, originalBaselineValue: obs.value, originalBaselineDate: obs.clinicalDate, deletedAt: new Date().toISOString(), note: "Baseline source was deleted. No replacement baseline was auto-assigned." });
